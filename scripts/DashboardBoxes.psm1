# ─────────────────────────────────────────────────────────────────────────────
# DashboardBoxes.psm1 — Box-drawing primitives, progress bars, section renderers
#
# Usage:
#   Import-Module ./scripts/DashboardBoxes.psm1 -Force
#
...
function Write-ScoreBox {
    ...
    if ($Data.Scores) {
        foreach ($key in $Data.Scores.Keys) {
            ...
            if ($val -is [int] -or $val -is [double] -or $val -is [decimal]) {
                ...
                $visLen = 1 + $labelCol + 1 + $scoreCol + 2 + $barWidth
                ...
                Write-BoxLine -Content "$($script:cWhite)$label $colored" -Width $w -VisualLength (1 + $labelCol + 1 + $valStr.Length)
            }
        }
    }
    ...
    Write-BoxLine -Content "$($script:cWhite)$statusLabel $statusIcon$statusColor$statusText$($script:cReset)" -Width $w -VisualLength (1 + $labelCol + 1 + 4 + $statusText.Length)
...
function Write-BlockedDetails {
    ...
    foreach ($block in $BlockedDetails) {
        ...
        if ($block.Errors) {
            foreach ($errLine in $block.Errors) { Write-Host "      $($script:cYellow)$errLine$($script:cReset)" }
        }
        Write-Host ""
    }
...
Export-ModuleMember -Function @(
    'Get-ProgressBar',
    'Write-BoxTop', 'Write-BoxBottom', 'Write-BoxDivider',
